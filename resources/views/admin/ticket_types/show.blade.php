    ...
    <div class="page-wrapper">
        ...
        <div class="page-content">
            <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
                <div>
                    <h4 class="mb-3 mb-md-0">Ticket Types</h4>

                </div>
                <div class="d-flex align-items-center flex-wrap text-nowrap">

                    <a style="" href="{{url('/admin/ticket_types')}}"
            ...
            <div class="row">
                <div class="col-md-12 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            <h6 class="card-title">{{$ticket_type->name}}</h6>
                            @if($errors->any())
                                @include('errors')
                            @endif
                            @if (session('message'))
                ...
                <div class="modal-dialog modal-lg modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalCenterTitle">{{$ticket_type->name}}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-label="btn-close"></button>
                        </div>
                        <form method="post" enctype="multipart/form-data" action="{{url('/admin/ticket_types/'
                        .$ticket_type->id)
                        }}">
                            @csrf
                            @method('PATCH')
                            <div class="modal-body">
                                <div class="card">
                                    <div class="card-body">
                                        <h6 class="card-title">Edit {{$ticket_type->name}}</h6>

                                        <div class="mb-3">
                                            <label for="colFormLabel" class="form-label">service name</label>
                                            <input name="name" type="text" value="{{$ticket_type->name}}"
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <textarea name="description" class="form-control"
                                                      id="colFormLabel">{{$ticket_type->description}}</textarea>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="modal-dialog modal-lg modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <form method="post" enctype="multipart/form-data"
                              action="{{url('/admin/ticket_types/'.$ticket_type->id) }}">
                            @csrf
                            @method('DELETE')
                            <div class="modal-body">
                                <div class="card">
                                    <div class="card-body">
                                        <h6 class="card-title">Delete {{$ticket_type->name}} ? </h6>
